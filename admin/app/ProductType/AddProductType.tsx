...
const validationSchema = Yup.object({
    name: Yup.string()
        ...
        .min(2, 'Product type name must be at least 2 characters'),
    status: Yup.string().required('Status is required'),
});

const AddProductType: FC<AddProductTypeProps> = ({ productTypeId, onSuccess, onCancel }) => {
    ...
    useEffect(() => {
        if (productTypeId !== undefined && productTypeId !== null) {
            ...
            fetchProductTypeById(id);
            setIsEditMode(true);
        }
    }, [productTypeId]);

    const fetchProductTypeById = async (id: number | string) => {
        setIsLoading(true);
        try {
            const response = await apiService.getById(API_ENDPOINTS.PRODUCT_TYPE, id);
            if (response.success && response.data) {
                ...
                toast.error(response.error || 'Failed to fetch product type data');
            }
        } catch (error) {
    ...
    const handleSubmit = async (values: FormValues, { setSubmitting, resetForm }: any) => {
        try {
            ...
            if (isEditMode && productTypeId !== undefined && productTypeId !== null) {
                const id = Number(productTypeId)
                if (!Number.isFinite(id) || id <= 0) {
                    toast.error('Invalid product type id')
                    setSubmitting(false)
                ...
                response = await apiService.update(API_ENDPOINTS.PRODUCT_TYPE, id, payload);
            } else {
                response = await apiService.create(API_ENDPOINTS.PRODUCT_TYPE, payload);
            }

            if (response.success) {
                toast.success(`Product type ${isEditMode ? 'updated' : 'created'} successfully!`);
                if (!isEditMode) {
                    resetForm();
                ...
                toast.error(response.error || `Failed to ${isEditMode ? 'update' : 'create'} product type`);
            }
        } catch (error) {
            console.error('Error submitting form:', error);
            toast.error(`Failed to ${isEditMode ? 'update' : 'create'} product type`);
        } finally {
            setSubmitting(false);
        }
    };
    ...
    if (isLoading) {
        return (
            <div className="form-container">
                <div className="text-center py-10">Loading product type data...</div>
            </div>
        );
    }
                    ...
                    <Form className="custom-form">
                        <div className="form-row form-row-1">
                            <div className="form-field">
                                ...
                                <Field type="text" id="name" name="name" placeholder="Enter product type name" />
                                <ErrorMessage name="name" component="div" className="error-message" />
                            </div>
                        </div>
                        <div className="form-row form-row-1">
                            <div className="form-field">
                                <label className="font-medium">Product Type Image</label>
                        ...
                        <div className="form-actions">
                            <button type="submit" className="btn-submit" disabled={isSubmitting}>
                                {isSubmitting ? (isEditMode ? 'Updating...' : 'Submitting...') : (isEditMode ? 'Update Product Type' : 'Submit Product Type')}
                            </button>
